            ...
            <div class="lg:col-span-2 space-y-6">
                ...
                <div class="bg-white dark:bg-gray-800 rounded-xl shadow-xs border border-gray-250 dark:border-gray-700 p-6 mt-6">
                    <div class="flex items-center justify-between mb-4">
                        ...
                        @php
                            $isOnAboutUs = $user->details->is_on_about_us ?? 0;
                        @endphp
                        <div class="flex items-center gap-3">
                            <span class="text-xs font-bold text-gray-700 dark:text-gray-400 uppercase tracking-wide">Show on About Us</span>
                            <button type="button" id="is-on-about-us-toggle" role="switch" aria-checked="{{ $isOnAboutUs == 1 ? 'true' : 'false' }}"
                                onclick="toggleIsOnAboutUs()"
                                class="relative inline-flex h-6 w-11 items-center rounded-full {{ $isOnAboutUs == 1 ? 'bg-[#008060]' : 'bg-gray-300 dark:bg-gray-600' }} transition-colors focus:outline-none focus:ring-2 focus:ring-[#008060] focus:ring-offset-1">
                                <span id="is-on-about-us-dot" class="inline-block h-4 w-4 {{ $isOnAboutUs == 1 ? 'translate-x-6' : 'translate-x-1' }} rounded-full bg-white shadow transition-transform duration-200"></span>
                            </button>
                            <input type="hidden" id="is-on-about-us" value="{{ $isOnAboutUs }}">
                        </div>
                    </div>
                    ...
                    <div class="space-y-4">
                        <div>
                            @php
                                $aboutUsType = $user->details->about_us_user_type ?? '';
                            @endphp
                            ...
                            <div class="relative">
                                <select id="about-us-user-type" class="w-full text-sm bg-[#f6f6f7] dark:bg-gray-700 border border-gray-300 dark:border-gray-650 text-gray-900 dark:text-gray-200 rounded-md px-3.5 py-2.5 focus:outline-none focus:ring-1 focus:ring-[#008060] appearance-none cursor-pointer">
                                    ...
                                    <option value="management" {{ strtolower($aboutUsType) === 'management' ? 'selected' : '' }}>Management</option>
                                    <option value="trainer" {{ strtolower($aboutUsType) === 'trainer' ? 'selected' : '' }}>Trainer</option>
                                    <option value="staff" {{ strtolower($aboutUsType) === 'staff' ? 'selected' : '' }}>Staff</option>
                                </select>
                        ...
                        <div>
                            ...
                            <textarea id="about-us-text-editor">{!! $user->details->about_us_text ?? '' !!}</textarea>
                        </div>
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <div>
                                ...
                                <div onclick="document.getElementById('about-us-image').click()"
                                    ...
                                    <p class="text-xs text-gray-500 dark:text-gray-400">Click to change about us image</p>
                                </div>
                                <input type="file" id="about-us-image" accept="image/*" class="hidden" onchange="previewAboutUsPhoto(this)">
                                @php
                                    $aboutUsPhotoUrl = '';
                                    if (!empty($user->details->about_us_image)) {
                                        $aboutUsPhotoUrl = str_starts_with($user->details->about_us_image, 'http') ? $user->details->about_us_image : Storage::disk('s3')->url($user->details->about_us_image);
                                    }
                                @endphp
                                <img id="about-us-photo-preview" class="{{ empty($aboutUsPhotoUrl) ? 'hidden' : '' }} mt-3 w-full rounded-lg object-cover max-h-40" src="{{ $aboutUsPhotoUrl }}" alt="Preview">
                            </div>
                            <div>
                                ...
                                <div onclick="document.getElementById('company-logo').click()"
                                    class="border-2 border-dashed border-gray-300 dark:border-gray-600 rounded-lg p-6 text-center cursor-pointer hover:border-[#008060] transition-colors bg-[#f6f6f7] dark:bg-gray-700">
                                    <svg class="w-8 h-8 text-gray-400 mx-auto mb-2" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z"/></svg>
                                    <p class="text-xs text-gray-500 dark:text-gray-400">Click to change logo</p>
                                </div>
                                <input type="file" id="company-logo" accept="image/*" class="hidden" onchange="previewCompanyLogo(this)">
                                @php
                                    $companyLogoUrl = '';
                                    if (!empty($user->details->company_logo)) {
                                        $companyLogoUrl = str_starts_with($user->details->company_logo, 'http') ? $user->details->company_logo : Storage::disk('s3')->url($user->details->company_logo);
                                    }
                                @endphp
                                <img id="company-logo-preview" class="{{ empty($companyLogoUrl) ? 'hidden' : '' }} mt-3 w-full rounded-lg object-contain max-h-40" src="{{ $companyLogoUrl }}" alt="Preview">
                            </div>
                        </div>
                    </div>
                </div>
