    ...
    <div class="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-6">
        <div>
            <div class="flex items-center gap-1.5 text-xxs font-bold uppercase tracking-wider text-gray-400 dark:text-gray-500 mb-1.5">
                ...
                <span>&rsaquo;</span>
                <span class="text-[#008060] font-extrabold">Edit Location Band</span>
            </div>
            <h1 class="text-2xl font-bold text-gray-900 dark:text-white">Edit Location Band</h1>
        </div>
    ...
    <div class="max-w-3xl">
        <div class="bg-white dark:bg-gray-800 rounded-xl shadow-xs border border-gray-250 dark:border-gray-700 overflow-hidden">
            ...
            <div class="px-6 py-4 border-b border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-800/80">
                <h2 class="text-sm font-bold text-gray-900 dark:text-white">Edit Location Band</h2>
            </div>
            ...
            <form onsubmit="handleSave(event, false)" class="p-6 space-y-5">
                ...
                <div class="grid grid-cols-1 md:grid-cols-4 gap-4 items-center">
                    ...
                    <div class="md:col-span-3">
                        <input type="text" id="band-name" value="{{ $band->location_band_name }}" required class="w-full text-sm bg-white dark:bg-gray-700 border border-gray-300 dark:border-gray-600 text-gray-900 dark:text-gray-200 rounded px-3 py-2 focus:outline-none focus:ring-1 focus:ring-[#008060] focus:border-[#008060]">
                    </div>
                </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-4 gap-4 items-center">
                    ...
                    <div class="md:col-span-3">
                        <select id="band-type" required class="w-1/2 text-sm bg-white dark:bg-gray-700 border border-gray-300 dark:border-gray-600 text-gray-900 dark:text-gray-200 rounded px-3 py-2 focus:outline-none focus:ring-1 focus:ring-[#008060] focus:border-[#008060]">
                            ...
                            <option value="Plus" {{ strtolower($band->location_band_type ?? '') == 'plus' ? 'selected' : '' }}>Plus</option>
                            <option value="Minus" {{ strtolower($band->location_band_type ?? '') == 'minus' ? 'selected' : '' }}>Minus</option>
                        </select>
                    </div>
                </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-4 gap-4 items-center">
                    ...
                    <div class="md:col-span-3">
                        <select id="band-venues" multiple="multiple" class="w-full md:w-1/2 text-sm bg-white dark:bg-gray-700 border border-gray-300 dark:border-gray-600 text-gray-900 dark:text-gray-200 rounded px-3 py-2 focus:outline-none focus:ring-1 focus:ring-[#008060] focus:border-[#008060]">
                            @php $selectedVenues = array_map('trim', explode(',', $band->venue ?? '')); @endphp
                            @foreach($venues as $v)
                                <option value="{{ $v->id }}" {{ in_array((string)$v->id, $selectedVenues) ? 'selected' : '' }}>{{ $v->venue_name }}</option>
                            @endforeach
                        </select>
                    </div>
                </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-4 gap-4 items-center">
                    ...
                    <div class="md:col-span-3">
                        <input type="number" id="band-adj" value="{{ $band->adjustment }}" required class="w-1/2 text-sm bg-white dark:bg-gray-700 border border-gray-300 dark:border-gray-600 text-gray-900 dark:text-gray-200 rounded px-3 py-2 focus:outline-none focus:ring-1 focus:ring-[#008060] focus:border-[#008060]">
                    </div>
                </div>
...
<script>
    async function handleSave(e, goBack) {
        ...
        try {
            const response = await fetch('/admin/course-price/location-bands/{{ $band->id }}', {
                method: 'PUT',
                headers: {
            ...
            if (result.success) {
                showToast("Location Band updated successfully!");
                if (goBack) {
