    ...
    <div class="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-6">
        <div>
            <div class="flex items-center gap-1.5 text-xxs font-bold uppercase tracking-wider text-gray-400 dark:text-gray-500 mb-1.5">
                ...
                <span>&rsaquo;</span>
                <span class="text-[#008060] font-extrabold">Accreditation Body</span>
            </div>
            <h1 class="text-2xl font-bold text-gray-900 dark:text-white">Accreditation Body</h1>
        </div>
        <a href="/admin/website/accreditation/create" class="inline-flex items-center justify-center text-sm font-semibold text-white bg-[#008060] hover:bg-[#006e52] px-5 py-2.5 rounded-md transition-all shadow-xs focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-[#008060] whitespace-nowrap">
            + Add
        </a>
    </div>
    ...
    <div class="bg-white dark:bg-gray-800 rounded-xl shadow-xs border border-gray-250 dark:border-gray-700 overflow-hidden">
        <div class="p-4 border-b border-gray-200 dark:border-gray-700 flex flex-col md:flex-row md:items-center justify-between gap-4 bg-gray-50 dark:bg-gray-800/80">
            ...
            <div class="flex items-center gap-2 w-full md:w-auto">
                ...
                <input type="text" id="accreditation-search" oninput="filterItems()" class="w-full md:w-64 text-sm bg-white dark:bg-gray-700 border border-gray-300 dark:border-gray-600 text-gray-900 dark:text-gray-200 rounded px-3 py-1.5 focus:outline-none focus:ring-1 focus:ring-[#008060]">
            ...
            <table class="w-full text-sm text-left">
                <thead class="text-xs font-bold text-gray-700 dark:text-gray-400 bg-[#f6f6f7] dark:bg-gray-900/40 uppercase border-b border-gray-250 dark:border-gray-700">
                    <tr>
                        ...
                        <th class="px-5 py-4 cursor-pointer hover:bg-gray-200 dark:hover:bg-gray-700 transition-colors" onclick="sortTable('name')">Name <span class="text-gray-400 ml-1">&#8693;</span></th>
                        <th class="px-5 py-4 cursor-pointer hover:bg-gray-200 dark:hover:bg-gray-700 transition-colors" onclick="sortTable('content')">Content <span class="text-gray-400 ml-1">&#8693;</span></th>
                        <th class="px-5 py-4 text-center w-32">Logo</th>
                        <th class="px-5 py-4 text-center w-20">Status</th>
                        <th class="px-5 py-4 text-center w-24">Actions</th>
                    </tr>
                </thead>
                <tbody id="accreditation-table-body" class="divide-y divide-gray-200 dark:divide-gray-700"></tbody>
            </table>
        </div>

        <div id="empty-state" class="hidden py-16 text-center">
            <h3 class="text-sm font-bold text-gray-900 dark:text-white">No accreditation bodies found</h3>
        </div>
...
<script>
    ...
    function filterItems() {
        ...
        filtered = items.filter(c => c.name.toLowerCase().includes(q) || c.content.toLowerCase().includes(q) || c.status.toLowerCase().includes(q));
    ...
    function clearSearch() { document.getElementById('accreditation-search').value = ''; filterItems(); }
    ...
    function renderTable() {
        const tbody = document.getElementById('accreditation-table-body');
        ...
        filtered.slice(start, end).forEach((item, idx) => {
            ...
            const logoCell = item.logo
                ? `<img src="${item.logo}" alt="${item.name}" class="h-14 w-auto max-w-[110px] object-contain mx-auto">`
                : `<div class="h-14 w-24 bg-gray-100 dark:bg-gray-700 rounded flex items-center justify-center text-gray-400 text-xxs mx-auto">No Image</div>`;
            const statusSwitch = `
            ...
            const shortContent = item.content.length > 90 ? item.content.substring(0, 90) + '...' : item.content;
            tr.innerHTML = `
                <td class="px-5 py-3 text-center font-medium">${start + idx + 1}</td>
                <td class="px-5 py-3 font-medium text-gray-900 dark:text-white">${item.name}</td>
                <td class="px-5 py-3 text-gray-600 dark:text-gray-400 max-w-xs">${shortContent}</td>
                <td class="px-5 py-3 text-center">${logoCell}</td>
                <td class="px-5 py-3 text-center">${statusSwitch}</td>
                <td class="px-5 py-3 text-center">
                    <div class="flex items-center justify-center gap-1">
                        <button onclick="deleteItem(${item.id})" class="text-red-500 hover:text-red-700 transition-colors p-1" title="Delete">
    ...
    function editItem(id) { window.location.href = `/admin/website/accreditation/${id}/edit`; }
