    ...
    <div class="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-6">
        <div>
            <div class="flex items-center gap-1.5 text-xxs font-bold uppercase tracking-wider text-gray-400 dark:text-gray-500 mb-1.5">
                ...
                <span>&rsaquo;</span>
                <span class="text-[#008060] font-extrabold">Edit Banner Slider</span>
            </div>
            <h1 class="text-2xl font-bold text-gray-900 dark:text-white">Edit Banner Slider</h1>
            <p class="text-xs text-gray-500 dark:text-gray-400 mt-0.5">Modify the details of an existing home page slider banner.</p>
        </div>
    ...
    <form onsubmit="handleSave(event)">
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
                ...
                <div class="bg-white dark:bg-gray-800 rounded-xl shadow-xs border border-gray-250 dark:border-gray-700 p-6">
                    ...
                    <div class="space-y-4">
                        ...
                        <div>
                            ...
                            <input type="url" id="banner-url" value="{{ $banner->url }}" placeholder="https://www.londontfe.com/..."
                        ...
                        <div>
                            ...
                            <input type="number" id="banner-sequence" value="{{ $banner->sequence }}" min="1" required placeholder="e.g. 1"
                                class="w-full text-sm bg-[#f6f6f7] dark:bg-gray-700 border border-gray-300 dark:border-gray-600 text-gray-900 dark:text-gray-200 rounded-md px-3.5 py-2.5 focus:outline-none focus:ring-1 focus:ring-[#008060] focus:border-[#008060] transition-colors">
                        </div>
                        ...
                        <div>
                            ...
                            <input type="text" id="alt-tag" value="{{ $banner->alt_tag }}" required placeholder="e.g. ChatGPT Course Banner"
                                class="w-full text-sm bg-[#f6f6f7] dark:bg-gray-700 border border-gray-300 dark:border-gray-600 text-gray-900 dark:text-gray-200 rounded-md px-3.5 py-2.5 focus:outline-none focus:ring-1 focus:ring-[#008060] focus:border-[#008060] transition-colors">
                        </div>
            ...
            <div class="space-y-6">
                ...
                <div class="bg-white dark:bg-gray-800 rounded-xl shadow-xs border border-gray-250 dark:border-gray-700 p-6">
                    <h2 class="text-sm font-bold text-gray-900 dark:text-white mb-1">Desktop Banner</h2>
                    ...
                    <img id="desktop-preview" class="mt-3 w-full rounded-lg object-contain max-h-32 bg-gray-50 dark:bg-gray-700 p-2 {{ $banner->image_url ? '' : 'hidden' }}" src="{{ $banner->image_url ?? '' }}" alt="Desktop Preview">
                </div>

                <!-- Mobile Banner Upload -->
                <div class="bg-white dark:bg-gray-800 rounded-xl shadow-xs border border-gray-250 dark:border-gray-700 p-6">
                    <h2 class="text-sm font-bold text-gray-900 dark:text-white mb-1">Mobile Banner</h2>
                    ...
                    <img id="mobile-preview" class="mt-3 w-full rounded-lg object-contain max-h-32 bg-gray-50 dark:bg-gray-700 p-2 {{ $banner->mobile_image_url ? '' : 'hidden' }}" src="{{ $banner->mobile_image_url ?? '' }}" alt="Mobile Preview">
                </div>
...
<script>
    ...
    function handleSave(e) {
        ...
        if (desktopInput.files && desktopInput.files[0]) {
            formData.append('image', desktopInput.files[0]);
        }
        if (mobileInput.files && mobileInput.files[0]) {
            formData.append('mobile_image', mobileInput.files[0]);
        }

        fetch('/admin/website/banners/{{ $banner->id }}', {
            method: 'POST',
            headers: {
