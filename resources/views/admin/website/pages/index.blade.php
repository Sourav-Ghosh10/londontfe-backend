    ...
    <div class="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-6">
        <div>
            <div class="flex items-center gap-1.5 text-xxs font-bold uppercase tracking-wider text-gray-400 dark:text-gray-500 mb-1.5">
                ...
                <span>&rsaquo;</span>
                <span class="text-[#008060] font-extrabold">Page Content</span>
            </div>
            <h1 class="text-2xl font-bold text-gray-900 dark:text-white">Content management</h1>
        </div>
        <a href="/admin/website/pages/create" class="inline-flex items-center justify-center text-sm font-semibold text-white bg-[#008060] hover:bg-[#006e52] px-5 py-2.5 rounded-md transition-all shadow-xs focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-[#008060] whitespace-nowrap">
            + Add content
        </a>
    </div>
        ...
        <div class="p-4 border-b border-gray-200 dark:border-gray-700 flex flex-col md:flex-row md:items-center justify-between gap-4 bg-gray-50 dark:bg-gray-800/80">
            ...
            <div class="flex flex-1 justify-end items-center gap-4 w-full md:w-auto">
                <div class="flex items-center gap-2">
                    ...
                    <select id="status-filter" onchange="filterPages()" class="text-sm bg-white dark:bg-gray-700 border border-gray-300 dark:border-gray-600 text-gray-900 dark:text-gray-200 rounded px-3 py-1.5 focus:outline-none focus:ring-1 focus:ring-[#008060]">
                ...
                <div class="flex items-center gap-2">
                    ...
                    <input type="text" id="pages-search" oninput="filterPages()" class="w-full md:w-64 text-sm bg-white dark:bg-gray-700 border border-gray-300 dark:border-gray-600 text-gray-900 dark:text-gray-200 rounded px-3 py-1.5 focus:outline-none focus:ring-1 focus:ring-[#008060]">
        ...
        <div class="overflow-x-auto">
            <table class="w-full text-sm text-left">
                <thead class="text-xs font-bold text-gray-700 dark:text-gray-400 bg-[#f6f6f7] dark:bg-gray-900/40 uppercase border-b border-gray-250 dark:border-gray-700">
                    <tr>
                        <th class="px-5 py-4 cursor-pointer hover:bg-gray-200 dark:hover:bg-gray-700 transition-colors" onclick="sortTable('title')">Title <span class="text-gray-400 ml-1">&#8693;</span></th>
                        ...
                        <th class="px-5 py-4 cursor-pointer hover:bg-gray-200 dark:hover:bg-gray-700 transition-colors" onclick="sortTable('menu_title')">Menu title <span class="text-gray-400 ml-1">&#8693;</span></th>
                ...
                <tbody id="pages-table-body" class="divide-y divide-gray-200 dark:divide-gray-700"></tbody>
            </table>
        </div>

        <div id="empty-state" class="hidden py-16 text-center">
            <h3 class="text-sm font-bold text-gray-900 dark:text-white">No pages found</h3>
        </div>
...
<script>
    ...
    function filterPages() {
        ...
        filtered = pages.filter(p => {
            const matchQ = p.title.toLowerCase().includes(q) || p.content.toLowerCase().includes(q) || p.menu_title.toLowerCase().includes(q);
    ...
    function sortTable(col) { sortCol === col ? sortAsc = !sortAsc : (sortCol = col, sortAsc = true); filterPages(); }
    function clearSearch() { document.getElementById('pages-search').value = ''; filterPages(); }
    ...
    function renderTable() {
        ...
        filtered.slice(start, end).forEach(item => {
            ...
            tr.innerHTML = `
                ...
                <td class="px-5 py-3 text-gray-500">${item.content.length > 50 ? item.content.substring(0, 50) + '...' : item.content}</td>
                <td class="px-5 py-3">${item.menu_title}</td>
                <td class="px-5 py-3">${statusSwitch}</td>
                <td class="px-5 py-3 text-center">
                    <div class="flex items-center justify-center gap-1">
                        <button onclick="deletePage(${item.id})" class="text-red-500 hover:text-red-700 transition-colors p-1" title="Delete">
